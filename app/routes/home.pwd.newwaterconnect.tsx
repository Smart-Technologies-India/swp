...
const NewWaterConnect: React.FC = (): JSX.Element => {
  ...
  const submit = async () => {
    ...
    type NewWaterConnectScheme = z.infer<typeof NewWaterConnectScheme>;

    const newWaterConnectScheme: NewWaterConnectScheme = {
    ...
    if (parsed.success) {
      ...
      if (
        ...
        const data = await ApiCall({
          query: `
              mutation createNewWaterConnect($createNewWaterConnectInput:CreateNewWaterConnectInput!){
                createNewWaterConnect(createNewWaterConnectInput:$createNewWaterConnectInput){
                    id
                  }
                }
              `,
          veriables: {
            createNewWaterConnectInput: {
              ...
              name: newWaterConnectScheme.name,
              address: newWaterConnectScheme.address,
              email: newWaterConnectScheme.email,
              mobile: newWaterConnectScheme.mobile,
              user_uid: newWaterConnectScheme.user_uid,
              village_id: newWaterConnectScheme.village_id,
              ward_number: newWaterConnectScheme.ward_number,
              diameter: newWaterConnectScheme.diameter,
              purpose: newWaterConnectScheme.purpose,
              ...
              iagree: newWaterConnectScheme.iagree,
              status: "ACTIVE",
              from_date: newWaterConnectScheme.from_date,
              connection_type: newWaterConnectScheme.connection_type,
        ...
        if (!data.status) {
          ...
          navigator(
            `/home/pwd/newwaterconnectview/${data.data.createNewWaterConnect.id}`
          );
        }
      } else {
    ...
    <>
      <div className="bg-white rounded-md shadow-lg p-4 my-4 w-full">
        ...
        <p className="text-center font-semibold text-xl text-gray-800">
          {" "}
          Format of Application for obtaining New Water Connection.{" "}
        </p>
...
export default NewWaterConnect;
