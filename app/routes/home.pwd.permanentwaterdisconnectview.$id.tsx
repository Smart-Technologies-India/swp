...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
  ...
  const data = await ApiCall({
    query: `
        query getPermanentWaterDisconnectById($id:Int!){
            getPermanentWaterDisconnectById(id:$id){
              id,
              name,
              address,
              mobile,
  ...
  const village = await ApiCall({
    query: `
      ...
      `,
    veriables: {
      id: parseInt(data.data.getPermanentWaterDisconnectById.village_id),
    },
  });

  return json({
    user: cookie,
    from_data: data.data.getPermanentWaterDisconnectById,
...
const PermanentWaterDisconnectView: React.FC = (): JSX.Element => {
  ...
  const submitQuery = async () => {
    ...
    const req: { [key: string]: any } = {
      stage: "PERMANENTWATERDISCONNECT",
    ...
    if (!addpayment.status) {
      ...
      if (data.status) {
        ...
        const savepaymentdoc = await ApiCall({
          query: `
                        mutation updatePermanentWaterDisconnectById($permanentWaterDisconnect:UpdatePermanentWaterDisconnectInput!){
                            updatePermanentWaterDisconnectById(permanentWaterDisconnect:$permanentWaterDisconnect){
                              id,
                            }
                          }
                        `,
    ...
    <>
      ...
      <div className="bg-white rounded-md shadow-lg p-4 my-4 w-full">
        <h1 className="text-gray-800 text-2xl font-semibold text-center">
          Permanent Water Connection Permission
        </h1>
        ...
        <p className="text-center font-semibold text-xl text-gray-800">
          {" "}
          SUBJECT : Request for Obtaining Permanent Water Connection Permission.{" "}
        </p>
        ...
        <div className="flex  flex-wrap gap-4 gap-y-2 px-4 py-2 my-2">
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700 ">
            <span className="mr-2">3.4</span> Diameter Of connection required
          </div>
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal">
        ...
        <div className="flex  flex-wrap gap-4 gap-y-2 px-4 py-2 my-2">
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700 ">
            <span className="mr-2">3.6</span> End Date
          </div>
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal">
        ...
        {isSubmited ? (
          user.id == from_data.userId ? (
            common.form_status == 75 ? (
              <Link
                target="_blank"
                to={`/permanentwaterdisconnectpdf/${encrypt(
                  `PWD-${("0000" + from_data.id).slice(-4)}-${
                ...
                className="py-1 w-full sm:w-auto text-white text-lg px-4 bg-[#0984e3] text-center rounded-md font-medium"
              >
                Download Permanenet Water Disconnect Certificate
              </Link>
            ) : null
          ) : (
            <>
                ...
                {common.query_status == "REJECTED" ? null : (
                  <>
                    ...
                    {common.form_status == 75 && user.id == 61 ? (
                      <Link
                        to={`/permanentwaterdisconnectpdf/${from_data.id}`}
                        className="py-1 w-full sm:w-auto text-white text-lg px-4 bg-cyan-500 text-center rounded-md font-medium"
                      >
...
export default PermanentWaterDisconnectView;
