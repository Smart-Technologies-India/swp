...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
  ...
  const data = await ApiCall({
    query: `
        query getDeathTeorById($id:Int!){
            getDeathTeorById(id:$id){
              id,
              name,
              address,
              mobile,
  ...
  const village = await ApiCall({
    query: `
      ...
      `,
    veriables: {
      id: parseInt(data.data.getDeathTeorById.village_id),
    },
  });
  ...
  const searchpayment = await ApiCall({
    ...
    veriables: {
      searchPaymentInput: {
        form_id: parseInt(data.data.getDeathTeorById.id),
        form_type: "DEATHTEOR",
      },
    },
  });
  return json({
    user: cookie,
    formid: id,
    from_data: data.data.getDeathTeorById,
...
const DeathTeorView = (): JSX.Element => {
      ...
      if (!addpayment.status) {
        ...
        if (data.status) {
          ...
          const savepaymentdoc = await ApiCall({
            query: `
                        mutation updateDeathTeorById($updateDeathTeorInput:UpdateDeathTeorInput!){
                            updateDeathTeorById(updateDeathTeorInput:$updateDeathTeorInput){
                              id,
                            }
                          }
                        `,
