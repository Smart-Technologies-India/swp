...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
  ...
  const data = await ApiCall({
    query: `
        query getMarriageTeorById($id:Int!){
            getMarriageTeorById(id:$id){
              id,
              name,
              address,
              mobile,
  ...
  const searchpayment = await ApiCall({
    ...
    veriables: {
      searchPaymentInput: {
        form_id: parseInt(data.data.getMarriageTeorById.id),
        form_type: "MARRIAGETEOR",
      },
    },
  });
...
const MarriageTeorView = (): JSX.Element => {
      ...
      if (!addpayment.status) {
        ...
        if (data.status) {
          ...
          const savepaymentdoc = await ApiCall({
            query: `
                        mutation updateMarriageTeorById($updateMarriageTeorInput:UpdateMarriageTeorInput!){
                            updateMarriageTeorById(updateMarriageTeorInput:$updateMarriageTeorInput){
                              id,
                            }
                          }
                        `,
            veriables: {
              updateMarriageTeorInput: payreq,
            },
          });
                ...
                {common.query_status == "REJECTED" ? null : (
                  <>
                    ...
                    {common.form_status == 50 && user.id == 5 ? (
                      <button
                        onClick={() => {
                          forwardRef!.current!.value = `The Marriage Teor documents requested as per application number ${from_data.id} pertaining to your request is as attached below.`;
                          setForwardBox((val) => true);
                          setNextData((val) => ({
...
export default MarriageTeorView;
