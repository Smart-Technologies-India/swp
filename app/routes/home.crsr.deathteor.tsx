...
const DeathTeor: React.FC = (): JSX.Element => {
  ...
  const submit = async () => {
    ...
    type DeathTeorScheme = z.infer<typeof DeathTeorScheme>;

    const deathTeorScheme: DeathTeorScheme = {
    ...
    const parsed = DeathTeorScheme.safeParse(deathTeorScheme);
    ...
    if (parsed.success) {
      ...
      if (
        ...
        const data = await ApiCall({
          query: `
              mutation createDeathTeor($createDeathTeorInput:CreateDeathTeorInput!){
                createDeathTeor(createDeathTeorInput:$createDeathTeorInput){
                    id
                  }
                }
              `,
          veriables: {
            createDeathTeorInput: {
              ...
              name: deathTeorScheme.name,
              address: deathTeorScheme.address,
              email: deathTeorScheme.email,
              mobile: deathTeorScheme.mobile,
              user_uid: deathTeorScheme.user_uid,
              village_id: deathTeorScheme.village_id,
              father_name: deathTeorScheme.father_name,
              mother_name: deathTeorScheme.mother_name,
              husband_name: deathTeorScheme.husband_name,
              place_of_death: deathTeorScheme.place_of_death,
              registration_number: deathTeorScheme.registration_number,
              ...
              iagree: deathTeorScheme.iagree,
              status: "ACTIVE",
              date_of_death: deathTeorScheme.date_of_death,
              date_of_birth: deathTeorScheme.date_of_birth,
              date_of_registration: deathTeorScheme.date_of_registration,
              gender: deathTeorScheme.gender,
            },
          },
        });
        if (!data.status) {
          ...
          navigator(
            `/home/crsr/deathteorview/${data.data.createDeathTeor.id}`
          );
        }
      } else {
    ...
    <>
      <div className="bg-white rounded-md shadow-lg p-4 my-4 w-full">
        <h1 className="text-gray-800 text-3xl font-semibold text-center">
          Death Teor Re-Issue Application
        </h1>
        ...
        <p className="text-center font-semibold text-xl text-gray-800">
          {" "}
          Format of Application for obtaining old Death Teor.{" "}
        </p>
