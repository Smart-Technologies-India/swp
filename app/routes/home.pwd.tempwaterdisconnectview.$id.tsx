...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
  ...
  const data = await ApiCall({
    query: `
        query getTempWaterDisconnectById($id:Int!){
            getTempWaterDisconnectById(id:$id){
              id,
              name,
              address,
              mobile,
  ...
  return json({
    user: cookie,
    from_data: data.data.getTempWaterDisconnectById,
...
const TempWaterDisconnectView: React.FC = (): JSX.Element => {
    ...
    if (!addpayment.status) {
      ...
      if (data.status) {
        ...
        const savepaymentdoc = await ApiCall({
          query: `
                        mutation updateTempWaterDisconnectById($updateTempWaterDisconnectInput:UpdateTempWaterDisconnectInput!){
                            updateTempWaterDisconnectById(updateTempWaterDisconnectInput:$updateTempWaterDisconnectInput){
                              id,
                            }
                          }
                        `,
          veriables: {
            updateTempWaterDisconnectInput: payreq,
          },
        });
    ...
    <>
      ...
      <div className="bg-white rounded-md shadow-lg p-4 my-4 w-full">
        <h1 className="text-gray-800 text-2xl font-semibold text-center">
          Temporary Water Disconnection Permission
        </h1>
        ...
        <p className="text-center font-semibold text-xl text-gray-800">
          {" "}
          SUBJECT : Request for Obtaining Temporary Water Disconnection
          Permission.{" "}
        </p>
...
export default TempWaterDisconnectView;
