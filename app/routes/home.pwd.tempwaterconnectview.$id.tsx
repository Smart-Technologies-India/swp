...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
  ...
  const data = await ApiCall({
    query: `
        query getTempWaterConnectById($id:Int!){
            getTempWaterConnectById(id:$id){
              id,
              name,
              address,
              mobile,
  ...
  const village = await ApiCall({
    query: `
      ...
      `,
    veriables: {
      id: parseInt(data.data.getTempWaterConnectById.village_id),
    },
  });

  return json({
    user: cookie,
    from_data: data.data.getTempWaterConnectById,
...
const TempWaterConnectView: React.FC = (): JSX.Element => {
    ...
    if (!addpayment.status) {
      ...
      if (data.status) {
        ...
        const savepaymentdoc = await ApiCall({
          query: `
                        mutation updateTempWaterConnectById($updateTempWaterConnectInput:UpdateTempWaterConnectInput!){
                            updateTempWaterConnectById(updateTempWaterConnectInput:$updateTempWaterConnectInput){
                              id,
                            }
                          }
                        `,
          veriables: {
            updateTempWaterConnectInput: payreq,
          },
        });
    ...
    <>
      ...
      <div className="bg-white rounded-md shadow-lg p-4 my-4 w-full">
        ...
        <p className="text-center font-semibold text-xl text-gray-800">
          {" "}
          SUBJECT : Request for Obtaining Temporary Water Connection Permission.{" "}
        </p>
        ...
        <div className="flex  flex-wrap gap-4 gap-y-2 px-4 py-2 my-2">
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700 ">
            <span className="mr-2">3.2</span> Diameter Of connection required
          </div>
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal">
            {from_data.diameter}
          </div>
        ...
        <div className="flex  flex-wrap gap-4 gap-y-2 px-4 py-2 my-2">
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700 ">
            <span className="mr-2">3.7</span> End Date
          </div>
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal">
        ...
        {isSubmited ? (
          user.id == from_data.userId ? (
            common.form_status == 75 ? (
              <Link
                target="_blank"
                to={`/tempwaterconnectpdf/${encrypt(
                  `TWC-${("0000" + from_data.id).slice(-4)}-${
                ...
                className="py-1 w-full sm:w-auto text-white text-lg px-4 bg-[#0984e3] text-center rounded-md font-medium"
              >
                Download Temporary Water Connect Certificate
              </Link>
            ) : null
          ) : (
            <>
                ...
                {common.query_status == "REJECTED" ? null : (
                  <>
                    ...
                    {common.form_status == 75 && user.id == 61 ? (
                      <Link
                        to={`/tempwaterconnectpdf/${from_data.id}`}
                        className="py-1 w-full sm:w-auto text-white text-lg px-4 bg-cyan-500 text-center rounded-md font-medium"
                      >
...
export default TempWaterConnectView;
