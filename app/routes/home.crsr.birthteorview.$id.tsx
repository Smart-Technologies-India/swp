...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
  ...
  const data = await ApiCall({
    query: `
        query getBirthTeorById($id:Int!){
            getBirthTeorById(id:$id){
              id,
              name,
              address,
              mobile,
  ...
  const searchpayment = await ApiCall({
    ...
    veriables: {
      searchPaymentInput: {
        form_id: parseInt(data.data.getBirthTeorById.id),
        form_type: "BIRTHTEOR",
      },
    },
  });
  return json({
    user: cookie,
    formid: id,
    from_data: data.data.getBirthTeorById,
...
const BirthTeorView = (): JSX.Element => {
      ...
      if (!addpayment.status) {
        ...
        if (data.status) {
          ...
          const savepaymentdoc = await ApiCall({
            query: `
                        mutation updateBirthTeorById($updateBirthTeorInput:UpdateBirthTeorInput!){
                            updateBirthTeorById(updateBirthTeorInput:$updateBirthTeorInput){
                              id,
                            }
                          }
                        `,
            veriables: {
              updateBirthTeorInput: payreq,
            },
          });
                ...
                {common.query_status == "REJECTED" ? null : (
                  <>
                    ...
                    {common.form_status == 50 && user.id == 51 ? (
                      <button
                        onClick={() => {
                          forwardRef!.current!.value = `The Birth Teor documents requested as per application number ${from_data.id} pertaining to your request is as attached below.`;
                          setForwardBox((val) => true);
                          setNextData((val) => ({
