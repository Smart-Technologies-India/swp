...
const WaterReconnect: React.FC = (): JSX.Element => {
  ...
  const reconnectPurposeRef = useRef<HTMLTextAreaElement>(null);
  ...
  const submit = async () => {
    const WaterReconnectScheme = z
      .object({
        ...
        purpose_of_reconection: z.string().nonempty("Enter Purpose of Reconnection"),
        purpose_of_disconnection: z.string().nonempty("Enter Purpose of Disconnection"),
    ...
    type WaterReconnectScheme = z.infer<typeof WaterReconnectScheme>;

    const waterReconnectScheme: WaterReconnectScheme = {
      ...
      purpose_of_disconnection: purposeRef!.current!.value,
      purpose_of_reconection: reconnectPurposeRef!.current!.value,
    ...
    const parsed = WaterReconnectScheme.safeParse(waterReconnectScheme);
    ...
    if (parsed.success) {
      ...
      if (
        ...
        const data = await ApiCall({
          query: `
              mutation createWaterReconnect($createWaterReconnectInput:CreateWaterReconnectInput!){
                createWaterReconnect(createWaterReconnectInput:$createWaterReconnectInput){
                    id
                  }
                }
              `,
          veriables: {
            createWaterReconnectInput: {
              ...
              name: waterReconnectScheme.name,
              address: waterReconnectScheme.address,
              email: waterReconnectScheme.email,
              mobile: waterReconnectScheme.mobile,
              user_uid: waterReconnectScheme.user_uid,
              village_id: waterReconnectScheme.village_id,
              ward_number: waterReconnectScheme.ward_number,
              meter_number: waterReconnectScheme.meter_number,
              purpose_of_disconnection: waterReconnectScheme.purpose_of_disconnection,
              purpose_of_reconection: waterReconnectScheme.purpose_of_reconection,
              ...
              iagree: waterReconnectScheme.iagree,
              status: "ACTIVE",
              
            
              connection_type: waterReconnectScheme.connection_type,
              entity_type: waterReconnectScheme.entity_type,
              muncipal_type: waterReconnectScheme.muncipal_type,
              ownership_type: waterReconnectScheme.ownership_type,
        ...
        if (!data.status) {
          ...
          navigator(
            `/home/pwd/waterreconnectview/${data.data.createWaterReconnect.id}`
          );
        }
      } else {
    ...
    <>
      <div className="bg-white rounded-md shadow-lg p-4 my-4 w-full">
        <h1 className="text-gray-800 text-3xl font-semibold text-center">
          Water Reconnection Application
        </h1>
        ...
        <p className="text-center font-semibold text-xl text-gray-800">
          {" "}
          Format of Application for Water Reconnection.{" "}
        </p>
        ...
        <div className="flex  flex-wrap gap-4 gap-y-2 px-4 py-2 my-2">
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700 ">
            <span className="mr-2">3.5</span> Disconnection Purpose
          </div>
          <div className="flex-none lg:flex-1 w-full lg:w-auto">
            <textarea
              ref={purposeRef}
              placeholder="Disconnection Purpose"
              className=" w-full border-2 border-gray-600 bg-transparent outline-none fill-none text-slate-800 p-2"
            ></textarea>
          </div>
        </div>
        ...
        <div className="flex  flex-wrap gap-4 gap-y-2 px-4 py-2 my-2">
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700 ">
            <span className="mr-2">3.6</span> Reconnection Purpose
          </div>
          <div className="flex-none lg:flex-1 w-full lg:w-auto">
            <textarea
