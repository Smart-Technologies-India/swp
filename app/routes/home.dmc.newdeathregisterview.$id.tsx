...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
  ...
  const data = await ApiCall({
    query: `
        query getDeathRegisterById($id:Int!){
            getDeathRegisterById(id:$id){
              id,
              name,
              address,
              mobile,
  ...
  const searchpayment = await ApiCall({
    ...
    veriables: {
      searchPaymentInput: {
        form_id: parseInt(data.data.getDeathRegisterById.id),
        form_type: "DEATHREGISTER",
      },
    },
  });
...
const DeathRegisterView = (): JSX.Element => {
    ...
    if (!addpayment.status) {
      ...
      if (data.status) {
        ...
        const savepaymentdoc = await ApiCall({
          query: `
                        mutation updateDeathRegisterById($updateDeathRegisterInput:UpdateDeathRegisterInput!){
                            updateDeathRegisterById(updateDeathRegisterInput:$updateDeathRegisterInput){
                              id,
                            }
                          }
                        `,
    ...
    <>
      ...
      <div className="bg-white rounded-md shadow-lg p-4 my-4 w-full">
        ...
        <div className="flex  flex-wrap gap-4 gap-y-2 px-4 py-2 my-2">
          ...
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal">
            {from_data.name_of_disease}
          </div>
        </div>
        <div className="flex  flex-wrap gap-4 gap-y-2 px-4 py-2 my-2">
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700 ">
            <span className="mr-2">3.15</span> Deceased Occupation
          </div>
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal">
            {from_data.deceased_occupation}
          </div>
        </div>
        <div className="flex  flex-wrap gap-4 gap-y-2 px-4 py-2 my-2">
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700 ">
        ...
        <div className="flex  flex-wrap gap-4 gap-y-2 px-4 py-2 my-2">
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700 ">
            <span className="mr-2">3.17</span> Attender Type
          </div>
          <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal">
                ...
                {common.query_status == "REJECTED" ? null : (
                  <>
                    ...
                    {common.form_status == 50 && user.id == 41 ? (
                      <button
                        onClick={() => {
                          forwardRef!.current!.value = `The Death Certificate requested as per application number ${from_data.id} pertaining to your is as attached below.`;
                          setForwardBox((val) => true);
                          setNextData((val) => ({
...
export default DeathRegisterView;
