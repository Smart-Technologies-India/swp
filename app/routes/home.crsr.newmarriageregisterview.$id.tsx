...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
    ...
    query: `
        query getMarriageRegisterById($id:Int!){
            getMarriageRegisterById(id:$id){
              ...
              date_of_marriage,
              joint_bride_groom_photo_url,
              applicant_uid_url,
              bride_uid_url,
              groom_uid_url,
              witness_one_signature_url,
              witness_two_signature_url,
              witness_three_signature_url,
              groom_signature_url,
              bride_signature_url,
              undertaking_url,
              iagree,
  ...
  // This code makes an API call to search for payment details related to a marriage register form.

  // Sending a query to the API using await to get the payment details
  const searchpayment = await ApiCall({
    ...
    veriables: {
      searchPaymentInput: {
        form_id: parseInt(data.data.getMarriageRegisterById.id),
        form_type: "MARRIAGEREGISTER",
      },
    },
  });
  ...
  return json({
    user: cookie,
    formid: id,
    from_data: data.data.getMarriageRegisterById,
...
const MarriageRegisterView = (): JSX.Element => {
  ...
  /**
   * Submits a form by making API calls to retrieve the user ID and create a new entry in the database.
   * @returns None
   */
  const submit = async () => {
  ...
  const submitQuery = async () => {
    /**
     * Submits a request with the given data to the server.
     * @param {RefObject<HTMLInputElement>} queryRef - The reference to the input element containing the query value.
     * @param {object} from_data - The data object containing the form information.
     * @param {object} user - The user object containing the user information.
     * @param {object} common - The common object containing common data.
     * @returns None
     */
    ...
    /**
     * Uploads a file attachment and sets the document URL in the request object.
     * @param {File} attachment - The file attachment to upload.
     * @returns None
     * @throws {Error} If unable to upload the attachment.
     */
    if (attachment != null) {
    ...
    <>
      ...
      <div className="bg-white rounded-md shadow-lg p-4 my-4 w-full">
        ...
        <>
          ...
          <div className="flex flex-wrap gap-4 gap-y-2 items-center px-4 py-2 my-2">
            <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700">
              <span className="mr-2">4.1</span>Groom's Signature Upload
              <p className="text-rose-500 text-sm">
                ( Maximum Upload Size 2MB & Allowed Format JPG / PDF / PNG )
              </p>
            </div>
          ...
          <div className="flex flex-wrap gap-4 gap-y-2 items-center px-4 py-2 my-2">
            <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700">
              <span className="mr-2">4.2</span>Bride's Signature Upload
              <p className="text-rose-500 text-sm">
                ( Maximum Upload Size 2MB & Allowed Format JPG / PDF / PNG )
              </p>
            </div>
          ...
          <div className="flex flex-wrap gap-4 gap-y-2 items-center px-4 py-2 my-2">
            <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700">
              <span className="mr-2">4.3</span> Joint Bride and Groom Photo
              Upload
              <p className="text-rose-500 text-sm">
                ( Maximum Upload Size 2MB & Allowed Format JPG / PDF / PNG )
              </p>
          ...
          <div className="flex flex-wrap gap-4 gap-y-2 items-center px-4 py-2 my-2">
            <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700">
              <span className="mr-2">4.1</span> Witness 1 Signature Upload
              <p className="text-rose-500 text-sm">
                ( Maximum Upload Size 2MB & Allowed Format JPG / PDF / PNG )
              </p>
            </div>
          ...
          <div className="flex flex-wrap gap-4 gap-y-2 items-center px-4 py-2 my-2">
            <div className="flex-none lg:flex-1 w-full lg:w-auto text-xl font-normal text-left text-gray-700">
              <span className="mr-2">4.2</span> Witness 2 Signature Upload
              <p className="text-rose-500 text-sm">
                ( Maximum Upload Size 2MB & Allowed Format JPG / PDF / PNG )
              </p>
            </div>
