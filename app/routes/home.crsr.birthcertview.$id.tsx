...
export const loader: LoaderFunction = async (props: LoaderArgs) => {
  ...
  const data = await ApiCall({
    query: `
        query getBirthCertificateById($id:Int!){
            getBirthCertificateById(id:$id){
              id,
              name,
              address,
              mobile,
  ...
  const searchpayment = await ApiCall({
    ...
    veriables: {
      searchPaymentInput: {
        form_id: parseInt(data.data.getBirthCertificateById.id),
        form_type: "BIRTHCERT",
      },
    },
  });
  ...
  const village = await ApiCall({
    query: `
      ...
      `,
    veriables: {
      id: parseInt(data.data.getBirthCertificateById.village_id),
    },
  });
  return json({
    user: cookie,
    formid: id,
    from_data: data.data.getBirthCertificateById,
...
const BirthCertificateView = (): JSX.Element => {
      ...
      if (!addpayment.status) {
        ...
        if (data.status) {
          ...
          const savepaymentdoc = await ApiCall({
            query: `
                        mutation updateBirthCertificateById($updateBirthCertificateInput:UpdateBirthCertificateInput!){
                            updateBirthCertificateById(updateBirthCertificateInput:$updateBirthCertificateInput){
                              id,
                            }
                          }
                        `,
            veriables: {
              updateBirthCertificateInput: payreq,
            },
          });
